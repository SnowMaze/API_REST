...
async function getToolStatus(department,status) {
    const [resultTools] = await pool.query("SELECT tools.*,categories.name as category "+
                                        "FROM tools LEFT JOIN categories ON tools.category_id = categories.id "+   
                                        "WHERE tools.owner_department = ? AND tools.status = ?", [department,status]);
    return resultTools[0];
}
async function getToolsByCostAndCategory(min_cost,max_cost,category_name) {
    const [resultTools] = await pool.query("SELECT  tools.*,categories.name as category  "+
                                        "FROM tools LEFT JOIN categories ON tools.category_id = categories.id"+
...
async function CreateTool(name, description, vendor, website_url, owner_department, monthly_cost, category_id) {
...
export { getTools, getTool, getToolStatus, getToolsByCostAndCategory, CreateTool,existsCategory };